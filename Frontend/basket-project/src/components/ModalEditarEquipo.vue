      ...
      <div class="flex items-center justify-center min-h-screen px-4 py-8">
        ...
        <div class="relative bg-white rounded-xl shadow-2xl w-full max-w-lg">
          ...
          <form @submit.prevent="handleSubmit">
            <div class="bg-gray-50 px-6 py-5 space-y-5 max-h-[65vh] overflow-y-auto">
              ...
              <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1">
                  Nivel de Competencia <span class="text-red-500">*</span>
                </label>
                <select v-model="form.id_nivel" required
                  class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 bg-white">
                  <option value="" disabled>Seleccionar nivel</option>
                  <option value="1">🎯 Amateur (jugadores 5-16 años)</option>
                  <option value="2">⭐ Profesional (jugadores 19+ años)</option>
                </select>
              </div>
              ...
              <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1">
                  Sede / Dirección <span class="text-red-500">*</span>
                </label>
                <input type="text" v-model="form.direccion_cancha" required
                  class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500"
              ...
              <div>
                ...
                <div v-if="!mostrarNuevaCancha">
                  ...
                  <p v-if="canchas.length === 0" class="text-xs text-gray-400 mt-1">
                    No hay canchas registradas aún
                  </p>
                </div>

                <div v-else class="space-y-3 border border-indigo-200 rounded-lg p-4 bg-indigo-50">
                  <h4 class="text-sm font-semibold text-indigo-800">Nueva cancha</h4>
                  ...
                  <input type="text" v-model="nuevaCancha.direccion" placeholder="Dirección *"
                    class="w-full px-3 py-2 text-sm border border-gray-300 rounded-md">
                  <input type="number" v-model="nuevaCancha.capacidad" placeholder="Capacidad (opcional)" min="0"
                    class="w-full px-3 py-2 text-sm border border-gray-300 rounded-md">
                </div>
              </div>
...
const form = ref({ nombre_oficial: '', siglas: '', id_clasificacion: '', id_nivel: '', direccion_cancha: '', id_cancha: '' })
...
const validarNombre = () => {
  const nombre = form.value.nombre_oficial.trim()
  if (!nombre) { errores.value.nombre = 'El nombre es requerido'; return }
  if (nombre.length < 3) { errores.value.nombre = 'Mínimo 3 caracteres'; return }
  const nombreNorm = nombre.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '')
  const tieneProhibida = PALABRAS_PROHIBIDAS.some(p => nombreNorm.includes(p))
  if (tieneProhibida) { errores.value.nombre = 'El nombre contiene palabras no permitidas'; return }
...
const resetForm = () => {
  form.value = { nombre_oficial: '', siglas: '', id_clasificacion: '', id_nivel: '', direccion_cancha: '', id_cancha: '' }
  ...
  if (newVal) {
    ...
    if (props.equipo) {
      form.value = {
        ...
        id_nivel: props.equipo.id_nivel ? String(props.equipo.id_nivel) : '',
...
const handleSubmit = async () => {
  ...
  if (!form.value.nombre_oficial || !form.value.siglas || !form.value.id_clasificacion || 
      !form.value.id_nivel || !form.value.direccion_cancha) {
  ...
  try {
    const dataToSend = {
      nombre_oficial: form.value.nombre_oficial.trim(),
      siglas: form.value.siglas.trim().toUpperCase(),
      id_clasificacion: parseInt(form.value.id_clasificacion),
      id_nivel: parseInt(form.value.id_nivel),
      direccion_cancha: form.value.direccion_cancha.trim(),
    }

    if (mostrarNuevaCancha.value && nuevaCancha.value.nombre_cancha) {
      if (!nuevaCancha.value.direccion) {
        errorMessage.value = 'La dirección de la nueva cancha es requerida'
        loading.value = false
        return
      }
      ...
      dataToSend.id_cancha = canchaResponse.data.cancha?.id_cancha || canchaResponse.data.id_cancha
